			      ...
			      struct ubi_fastmap_layout *fm)
{
	...
	ret = scan_pool(ubi, ai, fmpl1->pebs, pool_size, &max_sqnum, &freef);
	if (ret)
		goto fail;

	ret = scan_pool(ubi, ai, fmpl2->pebs, wl_pool_size, &max_sqnum, &freef);
	if (ret)
		goto fail;
